...
def train_table_q_learning(bad_mode, seed, vdn, n_runs=20, n_episodes=10000, save_dir="./saved_torchscript"):
    ...
    for run in range(n_runs):
        ...
        save_dir=f"./saved_q_tables/badmode_{bad_mode}_run_{run}.json"

        w0 = model.weights_0.detach().cpu().numpy().tolist()
        w1 = model.weights_1.detach().cpu().numpy().tolist()
        q_tables = {
            'weights_0': w0,
            'weights_1': w1
        }
        ...
        with open(save_dir, 'w') as f:
            json.dump(q_tables, f)
        print(f"Q-tables saved to {save_dir}")

        np.savetxt(f'./saved_q_tables//badmode_{bad_mode}_run_{run}-w0.txt', w0, fmt='%.6f', delimiter=' ')
        np.savetxt(f'./saved_q_tables//badmode_{bad_mode}_run_{run}-w1.txt', w1, fmt='%.6f', delimiter=' ')
